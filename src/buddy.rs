		...
		fn dialog(&self, dialog_type: DialogType) -> Vec<Vec<String>> {
			match dialog_type {
				DialogType::Chatter => vec![
					...
					vec!["HELLO!".to_string(), "IS THE AUTH LAYER STILL DISSOCIATED?".to_string(), "I MISS THEM".to_string()],
					vec!["INTERLOPER!".to_string(), "WELCOME".to_string(), "BUT ALSO PLEASE DO NOT BOTHER ME".to_string(), "VERY BUSY".to_string()],
				],
