...
use std::rc::Rc;
use glfw::Context;
use crate::font_manager::FontMan;
...
impl ChatterContext {
	...
	pub fn update_pos(&mut self) {
		if let Some(ref p) = self.parent {
			let parent_window_pos = Vec2::new_t(p.window.window_handle.get_pos());
			let parent_window_size = Vec2::new(parent_window_pos.x, parent_window_pos.y);
			let new_pos = (parent_window_pos + parent_window_size / 2.0
				+ self.parent_relative_pos.x
				- self.window_size / 2.0
			);
			self.window.window_handle.set_pos(new_pos.x as i32, new_pos.y as i32);
		}
	}
