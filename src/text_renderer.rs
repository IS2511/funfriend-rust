...
impl TextRenderer {
	pub fn new(text: String, font: String, sheet: BMSheet, width: i32, height: i32) -> Self {
		let shader_program = GL::shader("nop.frag", "nop.vert");
	...
	pub fn render(&self, dt: f64){
		unsafe{
			...
			gl::Uniform1i(gl::GetUniformLocation(self.shader_program, CString::new("texture1").unwrap().as_ptr()), 0);
			
			gl::BindVertexArray(self.vertex_array);
			gl::DrawElements(gl::TRIANGLES, 6 * self.text.len() as i32, gl::UNSIGNED_INT, ptr::null());
		}
	}
	...
	fn init_buffers(text: &str, sheet: &BMSheet, width: i32, height: i32) -> (GLuint, GLuint) {
		...
		let (vertices, indices) = FontMan::get_text_mesh(text, sheet, width / 2 - text_width / 2, height / 2 - text_height / 2, width, height);
