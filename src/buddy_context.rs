...
use rand::Rng;
use tracing::info;
use crate::buddy::funfriend::{Buddy, DialogType, FunfriendBuddy};
use crate::buddy_renderer::BuddyRenderer;
...
impl<'a> BuddyContext<'a> {
	pub fn new(buddy: &'a dyn Buddy, mut window: Window) -> Self {
		let renderer = BuddyRenderer::new(buddy);
		let window_size = Self::get_window_size(&renderer);
		
		window.window_handle.set_size(window_size.x as i32, window_size.y as i32);
	...
	pub fn render(&mut self, dt: f64){
		...
		self.renderer.render(dt, window_size.x as i32, window_size.y as i32);
	}
	
	pub fn goto(&mut self, pos: Vec2, dur: f64, set_as_static: bool){
		self.easing_t = 0.0;
		self.easing_dur = dur;
		self.easing_from = Vec2::new(self.window.window_handle.get_pos().0 as f64, self.window.window_handle.get_pos().1 as f64);
		self.easing_to = pos;
		...
		info!("going from {:?} to {:?}", self.easing_from, self.easing_to);
	}
	...
	pub fn update_wander(&mut self, dt: f64){
		if self.moving() {
			...
			self.window.window_handle.set_pos(new_position.x as i32, new_position.y as i32);
				...
				Behavior::Follow => {
					if !self.moving() {
						...
						if x_dist.abs() > FOLLOW_DIST as f64 {
							x_target = self.window.window_handle.get_pos().0 as f64 + x_dist - FOLLOW_DIST as f64 * x_dist.signum();
						}

						if y_dist.abs() > FOLLOW_DIST as f64 {
							y_target = self.window.window_handle.get_pos().1 as f64 + y_dist - FOLLOW_DIST as f64 * y_dist.signum();
						}
	...
	pub fn update_pos(&mut self, dt: f64) {
		...
		if self.held {
			...
			self.window.window_handle.set_pos(self.static_pos.x as i32, self.static_pos.y as i32);
		} else {
			self.held_timer -= dt;
			if self.held_timer <= 0.0 {
				...
				if self.waiting_for_stable_pos{
					...
					info!("travelled {:?}", stable_pos_dist);
