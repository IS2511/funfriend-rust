...
impl GL{
	pub fn buffer_data_array(target:GLenum, data: &[u8], usage_hint: GLenum){
		...
		unsafe{
			...
			gl::BufferSubData(target, 0, data_size as GLsizeiptr, data.as_ptr() as *const _);
		}
	}
	
	pub fn shader(fragment_filename: &str, vertex_filename: &str) -> GLuint {
		let vertex_shader_source = GL::load_shader_file(vertex_filename);
		let fragment_shader_source = GL::load_shader_file(fragment_filename);
		
		let vertex_shader = GL::compile_shader(vertex_shader_source, gl::VERTEX_SHADER);
		let fragment_shader = GL::compile_shader(fragment_shader_source, gl::FRAGMENT_SHADER);
		...
		unsafe {
			...
			if success != gl::TRUE as GLint {
				...
				error!("ERROR::PROGRAM::LINKING_FAILED\n{:?}", info_log);
			}
			program
		}
	}
